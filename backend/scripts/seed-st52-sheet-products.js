#!/usr/bin/env node
/**
 * Idempotent catalog load: ورق ST52 thickness×width Products via the API.
 * Does not rewrite Type identity/required/scope.
 * Length / supply form stay TRANSACTION and are not stored on the Product.
 *
 *   node backend/scripts/seed-st52-sheet-products.js
 */
import {
...
async function main() {
  ...
  const type = types.find((row) => row.name === ST52_SHEET_TYPE_NAME);
  if (!type) throw new Error('product type ورق ST52 not found');
  ...
  if (!thickness?.definition?.id || !width?.definition?.id) {
    throw new Error('ورق ST52 is missing thickness/width bindings');
  }
  for (const entry of [thickness, width]) {
    if (!entry.binding?.isRequired || entry.binding.valueScope !== 'PRODUCT') {
      throw new Error(`ورق ST52 ${entry.definition.code} must stay required PRODUCT identity — not rewriting schema`);
    }
  }
