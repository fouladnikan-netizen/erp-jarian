#!/usr/bin/env node
/**
 * Insert missing coil-rebar mills into Brand registry and bind them on
 * میلگرد کلاف. Reuses existing mill Brands (does not rename them).
 *
 *   node backend/scripts/seed-coil-rebar-brands.js
 */
import {
  COIL_REBAR_MILLS,
  STEEL_BRANDS,
} from '../src/domain/productMaster/steelBrandCatalog.js';
import { COIL_REBAR_TYPE_NAME } from '../src/domain/productMaster/coilRebarCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === COIL_REBAR_TYPE_NAME);
  ...
  for (const mill of COIL_REBAR_MILLS) {
