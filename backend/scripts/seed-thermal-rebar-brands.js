#!/usr/bin/env node
/**
 * Ensure thermal-rebar mill Brands exist and bind them on میلگرد حرارتی.
 * Reuses existing mill Brands (does not create duplicates).
 *
 *   node backend/scripts/seed-thermal-rebar-brands.js
 */
import {
  STEEL_BRANDS,
  THERMAL_REBAR_MILLS,
...
async function main() {
  ...
  const type = types.find((row) => row.name === TYPE_NAME);
  ...
  for (const mill of THERMAL_REBAR_MILLS) {
