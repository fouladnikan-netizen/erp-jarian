#!/usr/bin/env node
/**
 * Idempotent catalog load: ورق گالوانیزه thickness×width Products via the API.
 * Does not rewrite Type identity/required/scope.
 * Length, supply_form, and forming stay TRANSACTION and are not stored on the Product.
 *
 *   node backend/scripts/seed-galvanized-sheet-products.js
 */
import {
  GALVANIZED_SHEET_TYPE_NAME,
  galvanizedSheetIdentityRows,
} from '../src/domain/productMaster/galvanizedSheetCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === GALVANIZED_SHEET_TYPE_NAME);
  if (!type) throw new Error('product type ورق گالوانیزه not found');
  ...
  if (!thickness?.definition?.id || !width?.definition?.id) {
    throw new Error('ورق گالوانیزه is missing thickness/width bindings');
  }
  for (const entry of [thickness, width]) {
    if (!entry.binding?.isRequired || entry.binding.valueScope !== 'PRODUCT') {
      throw new Error(`ورق گالوانیزه ${entry.definition.code} must stay required PRODUCT identity — not rewriting schema`);
    }
  }
