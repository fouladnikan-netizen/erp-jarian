#!/usr/bin/env node
/**
 * Idempotent catalog load: سیم فولادی size Products via the API.
 * Does not rewrite Type bindings or SKUs.
 *
 *   node backend/scripts/seed-steel-wire-products.js
 */
import {
  STEEL_WIRE_GROUP_NAME,
  STEEL_WIRE_TYPE_NAME,
  steelWireIdentityRows,
} from '../src/domain/productMaster/steelWireCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => (
    row.name === STEEL_WIRE_TYPE_NAME
    && (row.groupName === STEEL_WIRE_GROUP_NAME || !row.groupName)
  )) || types.find((row) => row.name === STEEL_WIRE_TYPE_NAME);
  if (!type) throw new Error('product type سیم فولادی not found');
  ...
  if (!size?.definition?.id) {
    throw new Error('سیم فولادی is missing size binding');
  }
  if (!size.binding?.isRequired || size.binding.valueScope !== 'PRODUCT') {
    throw new Error('سیم فولادی size must stay required PRODUCT identity — not rewriting schema');
  }
