#!/usr/bin/env node
/**
 * Insert missing well-casing mills into Brand registry and bind them on
 * لوله جدار چاه. Reuses KALUP / NEYZAR — does not create duplicates.
 *
 *   node backend/scripts/seed-well-casing-pipe-brands.js
 */
import {
  STEEL_BRANDS,
  WELL_CASING_PIPE_MILLS,
} from '../src/domain/productMaster/steelBrandCatalog.js';
import { WELL_CASING_TYPE_NAME } from '../src/domain/productMaster/wellCasingPipeCatalog.js';
...
async function main() {
  ...
  if (!type) throw new Error('product type لوله جدار چاه not found');

  const duplicates = [];
  const created = [];
  const boundIds = [];

  for (const mill of WELL_CASING_PIPE_MILLS) {
