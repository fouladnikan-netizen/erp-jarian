#!/usr/bin/env node
/**
 * Idempotent catalog load: ورق A283 and ورق A36 thickness×width Products.
 * Same identity matrix as ورق ساده فولادی. Does not rewrite Type identity
 * or PRODUCT required/scope. Length / supply form stay TRANSACTION.
 *
 * Copies display-name rule and mill allow-list from ورق ساده فولادی.
 *
 *   node backend/scripts/seed-astm-hot-rolled-sheet-products.js
 */
import {
  ASTM_HOT_ROLLED_SHEET_TYPE_NAMES,
  HOT_ROLLED_SHEET_TYPE_NAME,
  hotRolledSheetIdentityRows,
} from '../src/domain/productMaster/hotRolledSheetCatalog.js';
...
async function main() {
  ...
  const sourceSummary = types.find((row) => row.name === HOT_ROLLED_SHEET_TYPE_NAME);
  if (!sourceSummary) throw new Error('product type ورق ساده فولادی not found');
  ...
  const summaries = [];
  for (const name of ASTM_HOT_ROLLED_SHEET_TYPE_NAMES) {
