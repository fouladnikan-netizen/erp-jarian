#!/usr/bin/env node
/**
 * Insert missing galvanized-pipe mills into Brand registry and bind them on
 * لوله گالوانیزه. Reuses SAVEH (نورد ساوه) — does not create a second ساوه.
 *
 *   node backend/scripts/seed-galvanized-pipe-brands.js
 */
import {
  GALVANIZED_PIPE_MILLS,
  STEEL_BRANDS,
} from '../src/domain/productMaster/steelBrandCatalog.js';
import { GALVANIZED_PIPE_TYPE_NAME } from '../src/domain/productMaster/galvanizedPipeCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === GALVANIZED_PIPE_TYPE_NAME);
  if (!type) throw new Error('product type لوله گالوانیزه not found');

  const duplicates = [];
  const created = [];
  const boundIds = [];

  for (const mill of GALVANIZED_PIPE_MILLS) {
