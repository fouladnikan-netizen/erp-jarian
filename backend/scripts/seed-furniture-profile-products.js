#!/usr/bin/env node
/**
 * Idempotent catalog load: پروفیل مبلی width×height×thickness Products via the API.
 * Does not rewrite Type bindings. Length stays TRANSACTION.
 *
 *   node backend/scripts/seed-furniture-profile-products.js
 */
import {
  FURNITURE_PROFILE_TYPE_NAME,
  furnitureProfileIdentityRows,
} from '../src/domain/productMaster/furnitureProfileCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === FURNITURE_PROFILE_TYPE_NAME);
  if (!type) throw new Error('product type پروفیل مبلی not found');
  ...
  if (!width?.definition?.id || !height?.definition?.id || !thickness?.definition?.id) {
    throw new Error('پروفیل مبلی is missing width_profile/length_profile/thickness bindings');
  }
  for (const entry of [width, height, thickness]) {
    if (!entry.binding?.isRequired || entry.binding.valueScope !== 'PRODUCT') {
      throw new Error(`پروفیل مبلی ${entry.definition.code} must stay required PRODUCT identity — not rewriting schema`);
    }
  }
  ...
  await req('PATCH', `/product-taxonomy/types/${type.id}`, token, {
    displayNameRule: {
      ...
      tokens: [
        ...
        { order: 1, sourceType: 'attribute', attributeId: thickness.definition.id, includeUnit: true, includeLabel: true },
        { order: 2, literalId: 'dims', sourceType: 'literal' },
        { order: 3, sourceType: 'attribute', attributeId: width.definition.id, includeUnit: false, includeLabel: false },
        { order: 4, literalId: 'times', sourceType: 'literal' },
        { order: 5, sourceType: 'attribute', attributeId: height.definition.id, includeUnit: false, includeLabel: false },
        { order: 6, literalId: 'branch', sourceType: 'literal' },
        ...(length?.definition?.id
          ? [{ order: 7, sourceType: 'attribute', attributeId: length.definition.id, includeUnit: true, includeLabel: false }]
          : []),
      ],
    },
  });
