#!/usr/bin/env node
/**
 * Idempotent catalog load: سپری size Products via the API.
 * Does not rewrite Type bindings. Length stays TRANSACTION.
 *
 *   node backend/scripts/seed-tee-products.js
 */
import {
  TEE_TYPE_NAME,
  teeIdentityRows,
} from '../src/domain/productMaster/teeCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === TEE_TYPE_NAME);
  if (!type) throw new Error('product type سپری not found');
  ...
  if (!size?.definition?.id) {
    throw new Error('سپری is missing size binding');
  }
  if (!size.binding?.isRequired || size.binding.valueScope !== 'PRODUCT') {
    throw new Error('سپری size must stay required PRODUCT identity — not rewriting schema');
  }
