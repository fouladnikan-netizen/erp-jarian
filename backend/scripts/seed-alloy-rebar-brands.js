#!/usr/bin/env node
/**
 * Ensure the mill Brands exist and bind them on میلگرد آلیاژی.
 * Reuses existing mill Brands (does not create duplicates).
 *
 *   node backend/scripts/seed-alloy-rebar-brands.js
 */
import {
  ALLOY_REBAR_MILLS,
  STEEL_BRANDS,
} from '../src/domain/productMaster/steelBrandCatalog.js';
import { ALLOY_REBAR_TYPE_NAME } from '../src/domain/productMaster/alloyRebarCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === ALLOY_REBAR_TYPE_NAME);
  ...
  for (const mill of ALLOY_REBAR_MILLS) {
