#!/usr/bin/env node
/**
 * Idempotent catalog load: ورق Ck45 thickness×width Products via the API.
 * Does not rewrite Type identity/required/scope.
 * Length / supply form stay TRANSACTION and are not stored on the Product.
 *
 *   node backend/scripts/seed-ck45-sheet-products.js
 */
import {
  CK45_SHEET_TYPE_NAME,
  ck45SheetIdentityRows,
} from '../src/domain/productMaster/ck45SheetCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === CK45_SHEET_TYPE_NAME);
  if (!type) throw new Error('product type ورق Ck45 not found');
  ...
  if (!thickness?.definition?.id || !width?.definition?.id) {
    throw new Error('ورق Ck45 is missing thickness/width bindings');
  }
  for (const entry of [thickness, width]) {
    if (!entry.binding?.isRequired || entry.binding.valueScope !== 'PRODUCT') {
      throw new Error(`ورق Ck45 ${entry.definition.code} must stay required PRODUCT identity — not rewriting schema`);
    }
  }
