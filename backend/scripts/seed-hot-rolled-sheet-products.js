#!/usr/bin/env node
/**
 * Idempotent catalog load: ورق ساده فولادی thickness×width Products via the API.
 * Does not rewrite Type identity/required/scope.
 * Length / supply form stay TRANSACTION and are not stored on the Product.
 *
 *   node backend/scripts/seed-hot-rolled-sheet-products.js
 */
import {
...
async function main() {
  ...
  const type = types.find((row) => row.name === HOT_ROLLED_SHEET_TYPE_NAME);
  if (!type) throw new Error('product type ورق ساده فولادی not found');
  ...
  if (!thickness?.definition?.id || !width?.definition?.id) {
    throw new Error('ورق ساده فولادی is missing thickness/width bindings');
  }
  for (const entry of [thickness, width]) {
    if (!entry.binding?.isRequired || entry.binding.valueScope !== 'PRODUCT') {
      throw new Error(`ورق ساده فولادی ${entry.definition.code} must stay required PRODUCT identity — not rewriting schema`);
    }
  }
