#!/usr/bin/env node
/**
 * Idempotent catalog load: لوله API size×thickness Products via the API.
 * Does not rewrite Type identity/required/scope.
 * Length stays TRANSACTION. unitWeight is kg per 6 m branch.
 *
 *   node backend/scripts/seed-api-pipe-products.js
 */
import {
  API_PIPE_TYPE_NAME,
  apiPipeIdentityRows,
} from '../src/domain/productMaster/apiPipeCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === API_PIPE_TYPE_NAME);
  if (!type) throw new Error('product type لوله API not found');
  ...
  if (!size?.definition?.id || !thickness?.definition?.id) {
    throw new Error('لوله API is missing size_pipe/thickness bindings');
  }
  for (const entry of [size, thickness]) {
    if (!entry.binding?.isRequired || entry.binding.valueScope !== 'PRODUCT') {
      throw new Error(`لوله API ${entry.definition.code} must stay required PRODUCT identity — not rewriting schema`);
    }
  }
  ...
  for (const row of rows) {
    ...
    if (!sameWeight(product.unitWeight, row.unitWeight)) {
    ...
    if (samples.length < 3 || rows.indexOf(row) === rows.length - 1) {
      samples.push(`${product.generatedName} ${product.unitWeight}kg`);
    }
    console.log(known ? 'reuse' : 'create', product.generatedName, product.sku, product.unitWeight);
  }
