#!/usr/bin/env node
/**
 * Bind water-test-pipe mills on لوله تست آب. Reuses existing mill Brands
 * (SAVEH / SEPANTA / SEPAHAN / KALUP / QAZVIN) — does not create duplicates.
 *
 *   node backend/scripts/seed-water-test-pipe-brands.js
 */
import {
  STEEL_BRANDS,
  WATER_TEST_PIPE_MILLS,
} from '../src/domain/productMaster/steelBrandCatalog.js';
import { WATER_TEST_PIPE_TYPE_NAME } from '../src/domain/productMaster/waterTestPipeCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === WATER_TEST_PIPE_TYPE_NAME);
  if (!type) throw new Error('product type لوله تست آب not found');

  const duplicates = [];
  const created = [];
  const boundIds = [];

  for (const mill of WATER_TEST_PIPE_MILLS) {
