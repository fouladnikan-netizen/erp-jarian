#!/usr/bin/env node
/**
 * Idempotent catalog load: ورق اسید شویی thickness×width Products via the API.
 * Does not rewrite Type identity/required/scope.
 * Length / supply form stay TRANSACTION and are not stored on the Product.
 *
 *   node backend/scripts/seed-pickled-sheet-products.js
 */
import {
  PICKLED_SHEET_TYPE_NAME,
  pickledSheetIdentityRows,
} from '../src/domain/productMaster/pickledSheetCatalog.js';
...
async function main() {
  ...
  const type = types.find((row) => row.name === PICKLED_SHEET_TYPE_NAME);
  if (!type) throw new Error('product type ورق اسید شویی not found');
  ...
  if (!thickness?.definition?.id || !width?.definition?.id) {
    throw new Error('ورق اسید شویی is missing thickness/width bindings');
  }
  for (const entry of [thickness, width]) {
    if (!entry.binding?.isRequired || entry.binding.valueScope !== 'PRODUCT') {
      throw new Error(`ورق اسید شویی ${entry.definition.code} must stay required PRODUCT identity — not rewriting schema`);
    }
  }
