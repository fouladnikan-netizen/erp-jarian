...
function withRequestId(req, body) {
...
export function errorHandler(err, req, res, _next) {
  if (err instanceof AppError) {
    ...
    return res.status(err.status).json(withRequestId(req, {
      error: err.code,
      message: err.message,
  ...
  logError(err, req);
  if (err?.code === '23505') {
    return res.status(409).json(withRequestId(req, {
      error: 'DUPLICATE',
  ...
  res.status(status).json(withRequestId(req, {
