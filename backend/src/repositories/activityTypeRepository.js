/**
 * Activity Type Registry PostgreSQL adapter — SQL only. No Zod / RBAC / HTTP.
 * Gap 1 — Shirazeh-driven canonical Activity type list.
 */
...
export async function list({ includeInactive = true } = {}, client = null) {
  ...
  const res = await run(
    `SELECT * FROM activity_type_registry ${where} ORDER BY sort_order ASC, key ASC`,
  );
...
export async function findByKey(key, client = null) {
  const run = runner(client);
  const res = await run(`SELECT * FROM activity_type_registry WHERE key = $1`, [key]);
...
export async function create(row, client = null) {
  const run = runner(client);
  const res = await run(
    `INSERT INTO activity_type_registry (key, label_fa, sort_order, is_active, created_by, updated_by)
     VALUES ($1, $2, $3, $4, $5, $5)
     RETURNING *`,
...
export async function update(key, patch, actorUserId, client = null) {
  const run = runner(client);
  const res = await run(
    `UPDATE activity_type_registry SET
