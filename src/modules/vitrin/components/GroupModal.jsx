...
export default function GroupModal({ onClose, onSubmit }) {
  ...
  return (
    <div className="vitrin-modal-overlay" onClick={onClose} role="presentation">
      <div className="vitrin-modal" role="dialog" aria-modal="true" aria-label="ثبت گروه کالا" onClick={(e) => e.stopPropagation()}>
        <header className="vitrin-modal__header">
          <h2 className="vitrin-modal__title">ثبت گروه کالا</h2>
        ...
        <form className="vitrin-modal__body" onSubmit={handleSubmit}>
          <label className="vitrin-form__field">
            <span className="vitrin-form__label">نام گروه کالا</span>
            <input
              type="text"
              value={name}
              onChange={(e) => { setName(e.target.value); setError(''); }}
              placeholder="مثلاً میلگرد"
              autoFocus
            />
          </label>
          ...
          <footer className="vitrin-modal__footer">
            ...
            <button type="submit" className="btn btn--primary">ثبت گروه</button>
          </footer>
        </form>
      </div>
    </div>
